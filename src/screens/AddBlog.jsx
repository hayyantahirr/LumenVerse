...
const AddBlog = () => {
  ...
  const handleTitleChange = (e) => {
    ...
    if (charCountWithoutSpaces <= maxCharsForTitle) {
    ...
    if (length > maxCharsForTitle) {
      // cut extra chars but keep spaces untouched
      let trimmed = lettersOnly.slice(0, maxCharsForTitle);
      // rebuild value with spaces but limited
      let newValue = "";
      let count = 0;
      for (let char of value) {
        if (char !== " ") {
          if (count < maxCharsForTitle) {
            newValue += char;
            count++;
          }
        } else {
  ...
  const handleSubTextChange = (e) => {
    ...
    if (charCountWithoutSpaces <= maxCharsForSubtext) {
      setSubtextCharCount(charCountWithoutSpaces);
      setSubTextValid(charCountWithoutSpaces >= 15); // or whatever your min length
      subtext.current.value = value;
    } else {
      // Prevent extra input if over the limit
      e.target.value = subtext.current.value;
    }
    ...
    // If exceeds max → cut it
    if (length > maxCharsForSubtext) {
      // cut extra chars but keep spaces untouched
      let trimmed = lettersOnly.slice(0, maxCharsForSubtext);
      // rebuild value with spaces but limited
      let newValue = "";
      let count = 0;
      for (let char of value) {
        if (char !== " ") {
          if (count < maxCharsForSubtext) {
            newValue += char;
            count++;
          }
        } else {
    ...
    // set states
    setSubtextCharCount(length);

    if (length >= 15) {
      setSubTextValid(true);
    } else {
      setSubTextValid(false);
    }
  };
  ...
  return (
    <>
      <form
        action=""
        className=" flex flex-col gap-3 w-full md:w-4/5  mx-auto justify-center items-center mt-15"
        onSubmit={submitBlog}
      >
          ...
          <div className="relative w-full">
            ...
            <span
              className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs ${
                charCount >= maxCharsForTitle ? "text-red-500" : "text-gray-500"
              }`}
            >
              {charCount}/{maxCharsForTitle}
            </span>
          </div>
        ...
        <div className="w-[80%] p-5 bg-gray-300 rounded-lg font-mono">
          ...
          <div className="relative w-full">
            ...
            <span
              className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs ${
                subtextCharCount >= maxCharsForSubtext
                  ? "text-red-500"
                  : "text-gray-500"
              }`}
            >
